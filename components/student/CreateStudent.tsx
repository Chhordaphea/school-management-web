...
const CreateStudent = () => {
    ...
    const createMutation = useMutation((data: any) => studentService.createStudent(data), {
        onMutate: () => {
        ...
        onError: (error: any) => {
            toast.error("Fail to create student");
        }
    });

    const updateMutation = useMutation((data: any) => studentService.updateStudent(data,updateData?.id), {
        onMutate: () => {
            toast.loading("Update Student...")
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['students']);
                ...
                <div className="modal-content custom_mod_comp">
                    <div className="custom_mod_body custom_d_flex custom_flex_col">
                        <div className="custom_d_flex custom_jt_cont_betw custom_alg_itm_ctr custom_p20 custom_brd_btm">
                            <label className="custom_lbl custom_fs16 custom_fw_semi_bd">{isUpdate ? "Update Student" : "New Student"}</label>
